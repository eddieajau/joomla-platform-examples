 ...
 */
class JWebSocketProtocolHyBi implements JWebSocketProtocol
{
	private $socket;
	private $state = 0;
	...
	private function parse($length)
	{
		...
		if ($this->state === 0 && $bytes >= 1)
		{
			...
			else if ($this->frameOp === 8)
			{ // closing frame
			// This is a close frame
				$this->state = 0;
				throw new RuntimeException('Closing frame');
			}
			...
			else if ($this->frameOp !== 1 && $this->frameOp !== 2 && $this->frameOp !== 9)
			{ // Unused Ops
		...
		else if ($this->state === 3 && $bytes >= 8)
		{
			...
			// Read 64 Bit Length
			$hi = ($this->buffer[$this->bufferPos] << 24) + ($this->buffer[$this->bufferPos + 1] << 16) + ($this->buffer[$this->bufferPos + 2] << 8)
				+ $this->buffer[$this->bufferPos + 3];

			$lo = ($this->buffer[$this->bufferPos + 4] << 24) + ($this->buffer[$this->bufferPos + 5] << 16)
				+ ($this->buffer[$this->bufferPos + 6] << 8) + $this->buffer[$this->bufferPos + 7];
